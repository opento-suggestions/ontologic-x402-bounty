...
### 2026-07-27 — the delivery watcher double-delivered (found in field testing; fixed)

**Finding.** `redeem.ts`'s idempotency check was *current balance* ("alias holds wKEY → skip"). The moment a customer SPENT their wKEY on a stamp, their balance returned to 0 and every later watcher pass re-vended their historical receipt. Reconstructed from the full mirror ledger (9 mints · 5 paid receipts · 1 ORG-initiated Phase-1 smoke vend): **three unpaid re-deliveries** on 2026-07-27 — to `0.0.9646091` @ `1785174205`, `0.0.9794951` @ `1785181001`, `0.0.9795879` @ `1785182851`. One apparent anomaly proved legitimate: `0.0.9794612` @ `1785174206` was the *first* delivery for a Phase-1 receipt (`0xdda8…`) that had gone unredeemed for eight days — the redeemable right surviving, as designed.

**Fix.** Idempotency is now COUNT-based by public state: deliveries-ever (the contract's outgoing wKEY transfers per account) vs receipts-ever (paid memos per alias), netted oldest-first. Repeat purchases redeem correctly; nothing delivers twice; an undercounted receipt window errs toward skip, never over-vend. Validated dry-run against the live ledger: every alias nets to zero owed, including the receipt that the earlier unpaid re-delivery had already covered.

**Burn cadence, folded in.** The July 19 burn was `lane-b-smoke` step 6 — the only `burnCollected` caller in the codebase, which is why fees accumulated once the goose-era flow (redeem + agent stamps) replaced the smoke. A stamp cannot trigger the burn (it is a TopicMessageSubmit; no contract executes), so the watcher now ends every pass by burning the treasury's collected balance: vend and burn in the same custody and the same operational loop. Executed: `burnCollected(5)` @ `1785184757`, supply 8 → **3** — customer-held only, treasury zero.
